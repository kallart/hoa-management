        ...
        <div className="scale-transform-wrapper" style={isFullscreen ? { display: 'flex', justifyContent: 'center', width: '100%' } : { transform: `scale(${scale})`, transformOrigin: 'top center' }}>
          ...
          <div id="receipt-print-area" onClick={() => setIsFullscreen(!isFullscreen)} style={{
            ...
            <div style={{ marginBottom: '15px' }}>
              {/* Top Row: Logo, Address, and Receipt Box */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', marginLeft: '15px' }}>
                    <h1 style={{ fontSize: '1.15rem', fontWeight: 'bold', margin: '0 0 2px 0', color: 'black', whiteSpace: 'nowrap' }}>นิติบุคคลหมู่บ้านจัดสรร รอยัลราชาวดี</h1>
                    <p style={{ fontSize: '0.85rem', color: '#4B5563', margin: 0, lineHeight: '1.4', whiteSpace: 'nowrap' }}>
                      หมู่บ้านรอยัลราชาวดี ถนนพุทธบูชา แขวงบางมด<br/>เขตจอมทอง จ.กรุงเทพฯ 10150
                    </p>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid black', padding: '0 25px', borderRadius: '4px', height: '43px' }}>
                  <div style={{ fontWeight: 'bold', fontSize: '1.25rem', whiteSpace: 'nowrap' }}>ใบเสร็จรับเงิน</div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '20px' }}>
                <div style={{ fontSize: '1rem', fontWeight: 'bold', color: '#6B7280' }}>
                  อ้างอิงบิล: <span>{displayInvoiceNumber}</span>
                </div>
                <div style={{ fontSize: '0.95rem', fontWeight: 'bold', textAlign: 'right', whiteSpace: 'nowrap' }}>
                  เลขที่ใบเสร็จ: <span style={{ color: 'black' }}>{displayReceiptNumber}</span>
                </div>
              </div>
            </div>
              ...
              <tbody>
                {invoiceItemsFixed.map((item, index) => (
                  <tr key={index} style={{ height: '40px', borderBottom: item.isSummary ? '1px solid #D1D5DB' : '1px dashed #E5E7EB', backgroundColor: item.isSummary ? '#F3F4F6' : 'transparent' }}>
                    {item.isSummary ? (
                      <td colSpan={2} style={{ padding: '6px 8px', verticalAlign: 'top', fontWeight: 'bold', color: '#4B5563', paddingLeft: '26px' }}>
                        {item.name}
                      </td>
                    ) : (
                      <>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '40px', padding: '0 40px', marginBottom: '40px' }}>
              ...
              <div style={{ flex: 1, textAlign: 'center' }}>
                <p style={{ margin: '0 0 30px 0', fontSize: '0.95rem' }}>ลงชื่อ.......................................................ผู้รับเงิน</p>
            ...
            {payment.slipUrl && (
              <div className="no-print" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', borderTop: '1px dashed #D1D5DB', paddingTop: '20px' }}>
                ...
                <div 
                  ...
                  onClick={() => window.open(`http://localhost:3001${payment.slipUrl}`, '_blank')}
                  title="คลิกเพื่อดูรูปภาพขนาดเต็ม"
                >
                  <img 
                    src={`http://localhost:3001${payment.slipUrl}`} 
                    alt="Payment Slip" 
