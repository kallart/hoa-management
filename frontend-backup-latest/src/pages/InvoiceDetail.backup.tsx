...
const InvoiceDetail = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
      // Calculate available space to fit the bill optimally on screen
      // Account for the main application topbar (~70px) and some padding (~50px)
      const availableHeight = window.innerHeight - 120; 
      const availableWidth = window.innerWidth - 120; // Leave 120px for the right floating buttons
      
      const scaleHeight = availableHeight / 600;
      const scaleWidth = availableWidth / 850;
      
      // Use the minimum scale so it fits perfectly without overflowing
      const optimalScale = Math.min(scaleHeight, scaleWidth);
      
      // Set the scale (cap it at 2x max to prevent it being ridiculously huge on 4K screens)
      setScale(Math.min(optimalScale, 2));
    };
  ...
  const handleSavePdf = () => {
    // Native browser print to PDF is the only way to get pixel-perfect Thai fonts.
    // Changing the document title ensures "Save as PDF" defaults to the correct filename.
    const originalTitle = document.title;
    document.title = `Invoice_${invoice.invoiceNumber}`;
    window.print();
    document.title = originalTitle;
  ...
  const rawItems = [
    {
      name: `ค่าส่วนกลาง ปี ${thYear} ( ก.ค.${shortYear} - มิ.ย.${nextShortYear} )`,
      subtext: `= (${invoice.property.landArea.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})} x 42.- x 12 เดือน)`,
    ...
    {
      name: invoice.interest > 0 ? <span>ดอกเบี้ยยอดค้างชำระ <span style={{ fontSize: '0.95rem', color: '#4B5563', fontWeight: 'normal' }}>({invoice.arrears.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})} x 2% x 12)</span></span> : '',
      subtext: '',
      area: '',
      rate: '',
  ...
  return (
    <div className="page-container invoice-page-container" style={{ backgroundColor: '#F3F4F6', position: 'relative' }}>
      ...
      <div className="print-hide" style={{
        ...
        <button onClick={() => setIsFullscreen(true)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '70px', height: '70px', backgroundColor: 'white', border: '1px solid #E5E7EB', borderRadius: '12px', cursor: 'pointer', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', color: 'var(--color-primary)' }}>
          <ZoomIn size={22} />
          <span style={{ fontSize: '0.75rem', marginTop: '6px', fontWeight: 'bold' }}>เต็มจอ</span>
        </button>
        ...
        <div className="scale-transform-wrapper" style={isFullscreen ? { display: 'flex', justifyContent: 'center', width: '100%' } : { transform: `scale(${scale})`, transformOrigin: 'top center' }}>
          ...
          <div id="invoice-print-area" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '15px' }}>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '15px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', marginLeft: '40px' }}>
                  <h1 style={{ fontSize: '1.25rem', fontWeight: 'bold', margin: '0 0 2px 0', color: 'black' }}>นิติบุคคลหมู่บ้านจัดสรร รอยัลราชาวดี</h1>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '8px', fontSize: '1.05rem' }}>
              <thead>
                <tr style={{ backgroundColor: '#E5E7EB', borderBottom: '1px solid black' }}>
                  ...
                  <th style={{ padding: '8px', textAlign: 'center', width: '120px' }}>พื้นที่ (ตร.ว.)</th>
            ...
            <div style={{ display: 'flex', borderTop: '2px solid black', borderBottom: '2px solid black', marginBottom: '15px' }}>
              <div style={{ flex: 1, padding: '10px 15px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '1.05rem' }}>
                ({bahttext(invoice.amount)})
              </div>
              <div style={{ width: '160px', padding: '10px 15px', backgroundColor: '#E5E7EB', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '1.1rem', borderLeft: '1px solid #D1D5DB' }}>
                รวมเป็นเงิน
              </div>
            ...
            <div style={{ display: 'flex', gap: '15px' }}>
              {/* Bank Info */}
              <div style={{ flex: 1, border: '2px solid #D1D5DB', borderRadius: '12px', padding: '12px 20px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                <div style={{ textAlign: 'center', fontWeight: 'bold', fontSize: '1.1rem', marginBottom: '8px' }}>
                ...
                <ul style={{ listStyleType: 'none', padding: 0, margin: 0, lineHeight: '1.4', fontSize: '0.85rem' }}>
