...
const Payments = () => {
  ...
  const handleSubmitPayment = async () => {
    ...
    if (!amount) return alert('กรุณาระบุยอดเงินที่รับชำระ');
    if (!slipFile) return alert('กรุณาแนบรูปภาพหลักฐานการชำระเงินก่อนบันทึก');
    ...
    try {
      ...
      alert('บันทึกการชำระเงินเรียบร้อยแล้ว สถานะบิลถูกเปลี่ยนเป็น "รอตรวจสอบยอดเงิน"');
  ...
  const handleVerifyPayment = async (inv: Invoice, paymentIdToVerify?: string) => {
    if (!inv.payments || inv.payments.length === 0) return alert('ไม่พบข้อมูลการชำระเงิน');
    let paymentId = paymentIdToVerify;
    if (!paymentId) {
      const unverified = inv.payments.find(p => !p.receiptNumber);
      if (!unverified) return alert('ทุกยอดชำระถูกตรวจสอบแล้ว');
      paymentId = unverified.id;
    }
    if (confirm('ตรวจสอบสลิปและยืนยันการรับเงินใช่หรือไม่? ระบบจะออกใบเสร็จให้อัตโนมัติ')) {
      try {
        await axios.post(`http://localhost:3001/api/payments/${paymentId}/verify`);
        alert('ตรวจสอบยอดเงินและออกใบเสร็จเรียบร้อยแล้ว');
        fetchInvoices();
      } catch (error) {
        console.error(error);
        alert(error.response?.data?.error || 'เกิดข้อผิดพลาดในการออกใบเสร็จ');
      }
    }
  };
  ...
  const handleCancelPayment = async (inv: Invoice) => {
    ...
    if (confirm('คุณต้องการยกเลิกและลบข้อมูลนี้ใช่หรือไม่? ข้อมูลสลิปและใบเสร็จ (ถ้ามี) จะถูกลบออกทั้งหมด')) {
      try {
        ...
        alert('ยกเลิกการแจ้งชำระเรียบร้อยแล้ว');
        fetchInvoices();
      } catch (error: any) {
        console.error(error);
                      ...
                      <>
                        ...
                        {(() => {
                          ...
                          return latestSlipPayment ? (
                            <button 
                              onClick={() => setViewSlipUrl(`http://localhost:3001${latestSlipPayment.slipUrl}`)}
